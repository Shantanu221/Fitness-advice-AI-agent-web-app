...
def personal_data_form():
    with st.form("personal_data"):
        ...
        age = st.number_input("Age",min_value=2,max_value=110,step=1,value=profile["general"]["age"])
        ...
        if personal_data_submit:
            if all([name,age,gender,activity_level,weight,height]):
                with st.spinner():
                    #save the data
...
def notes():
    ...
    for i,note in enumerate(st.session_state.notes):
        col = st.columns([5,1]) ## col1 -> size=5, col2->size=1
        with col[0]:
